...
NS_LOG_COMPONENT_DEFINE("Zigbee-Final-Code");
...
const uint32_t ZIGBEE_CHANNEL_BASE_FREQ = 2405;  
const uint32_t ZIGBEE_CHANNEL_SPACING = 5;      

uint32_t GetChannelFrequency(uint32_t channel) {
  if (channel < 11 || channel > 26) {
    NS_LOG_ERROR("Invalid channel: " << channel << ". Valid range: 11-26");
    return ZIGBEE_CHANNEL_BASE_FREQ;  
  }
...
void GenerateSensorEvent(uint32_t nodeId, const std::string& nodeName,
  ...
  uint32_t packetSize = 64 + rand() % 32;  
...
int main(int argc, char* argv[]) {
  ...
  if (enableNetAnim) {
    ...
    for (uint32_t i = 1; i < numNodes; i++) {
      ...
      anim.UpdateNodeColor(g_allNodes.Get(i), 0, 255, 0);  
    }
  }
